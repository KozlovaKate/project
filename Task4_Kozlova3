...
def gauss(q, m, d_g, w_g, d_t, eps=1, mu=1, Sc=1):
    return np.sin(2*np.pi/100*(q*Sc - m*np.sqrt(eps*mu)))*np.exp(-((((q \
             - m*np.sqrt(eps*mu)/Sc) - (d_g / d_t)) / (w_g / dt)) ** 2))
...
# Функция деления без деления на ноль
def devision(A, B, q = -1):
    if B == 0:
        return 0
    elif q >= 0:
        return round(A/B, q)
...
eps = np.ones(maxSize)
eps[posL1min:posL2min] = 3.5
eps[posL2min:posL3min] = 4.8
eps[posL3min:] = 6.5
...
print(df)
# Частотная сетка
flist = np.arange(0, maxTime * df, df)
...
Ez1Spec = np.abs(fft(probe1Ez))
Ez0Spec = np.abs(fft(Ez0))
Gamma = [devision(Ez1Spec[n], Ez0Spec[n]) \
         for n in range(0, len(Ez1Spec))]

# Отображение графиков
fig, (ax1, ax2, ax3) = plt.subplots(3, 1)
# Сигналы
ax1.set_xlim(0, 0.06 * maxTime * dt)
